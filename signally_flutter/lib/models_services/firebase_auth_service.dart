...
import 'package:signally/models_services/firebase_image_service.dart';
...
/* --------------------- NOTE Update Appversion Login & dev token --------------------- */
  ...
  static Future<void> uploadProfilePicture({required File file}) async {
    ...
    _firestore.collection('users').doc(fbUser.uid).update({'profileImageUrl': url});
  }
...
/* ------------------------------ NOTE SIGNOUT ------------------------------ */
  ...
  static resetPassword(String email) async {
    try {
      ...
      ZUtils.showToastSuccess(message: 'Password reset email sent!', duration: Duration(seconds: 5));
      await Future.delayed(Duration(seconds: 5));

      if (Get.currentRoute == '/ResetPasswordPage') {
